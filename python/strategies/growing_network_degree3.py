...
class Growing_network_degree3(CN_Generator):
    ...
    def __init__(self, dataset, args=None, DSN=None):
        ...
        self.e = self.args.e
        self.b = self.args.b
        self.filename = "graph-%s-%s-%d-%d-%d.graphml" % (dataset, self.args.n, int(self.e), self.b, time.time())
        self._post_init()
    ...
    def stop_condition(self):
        return len(self.graph.nodes) >= self.n
    ...
    def add_links(self, new_node):
        ...
        if visible_links:
            ...
            if (self.graph.degree(link[1].gid) > 2):
                self.recovered.add(link[1])
                self.infected.remove(link[1])
