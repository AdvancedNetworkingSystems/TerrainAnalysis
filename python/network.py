    ...
    def add_link(self, link):
        ...
        for antenna in self.graph.nodes[link[1].gid]['antennas']:
            if antenna.check_node_vis(link_angles=link[4]):
                print("Node added to existing antenna")
                ant1 = antenna
                rate, device0 = ubnt.get_fastest_link_hardware(link[2], target=antenna.ubnt_device[0])
                break
        if not ant1:
            # TODO: find proper device and create new antenna
            # link is new negotitate both device
            rate, device1 = ubnt.get_fastest_link_hardware(link[2])
            device0 = device1
        ...
        self.graph.add_edge(link[0].gid, link[1].gid, loss=link[2], src_ant=ant0, dst_ant=ant1, rate=rate)
        self.graph.add_edge(link[1].gid, link[0].gid, loss=link[2], src_ant=ant1, dst_ant=ant0, rate=rate)
