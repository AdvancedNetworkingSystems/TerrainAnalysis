...
class Network():
    ...
    def add_node(self, building):
        self.graph.add_node(building.gid, pos=building.xy(), antennas=list(), cost=node_fixed_cost)
    ...
    def add_link(self, link):
        ...
        for antenna in self.graph.nodes[link[1].gid]['antennas']:
            if antenna.check_node_vis(link_angles=link[4]):
                ...
                ant1 = antenna
                rate0, device0 = ubnt.get_fastest_link_hardware(link[2], target=antenna.ubnt_device[0])
                rate0, rate1 = ubnt.get_maximum_rate(link[2], device0[0], antenna.ubnt_device[0])
                break
        if not ant1:
            ...
            rate0, rate1 = ubnt.get_maximum_rate(link[2], device1[0], device1[0])
            device0 = device1
        ...
        self.graph.add_edge(link[0].gid, link[1].gid, loss=link[2], src_ant=ant0, dst_ant=ant1, rate=rate0)
        self.graph.add_edge(link[1].gid, link[0].gid, loss=link[2], src_ant=ant1, dst_ant=ant0, rate=rate1)
